...
public class Evaluator
{
    ...
    private void ConstructEvaluator(EvaluatorItem[] items, Dictionary<string, string> listAssemblies = null!)
    {
        ...
        //创建C#编译器实例
        //ICodeCompiler comp = CodeDomProvider.CreateProvider("").CreateCompiler()!;
        ...
        //得到编译器实例的返回结果
        CompilerResults cr = csharpCodeProvider.CompileAssemblyFromSource(cp, code.ToString());
    ...
    private void ConstructEvaluatorByAllCode(string allcode, string _namespace, string _class, List<string> listAssemblies)
    {
        ...
        //创建C#编译器实例
        //ICodeCompiler comp = CSharpCodeProvider.CreateProvider("CSharp").CreateCompiler()!;
